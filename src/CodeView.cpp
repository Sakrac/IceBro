...
void CodeView::Draw(int index)
{
	...
	const Regs& regs = GetRegs();	// current registers
	...
	} else if (evalAddress||(fixedAddress && MemoryChange())) {
		SetAddr(ValueFromExpression(address+(fixedAddress ? 1 : 0)));
		evalAddress = false;
	}
	...
	if (active) {
		...
		if (ImGui::IsKeyPressed(GLFW_KEY_F9, false)) {
			// TODO: Mkae a better implementation of toggle vice break
			SetViceBP(addrCursor, addrCursor, GetViceBPIndex(addrCursor, VBP_Break),
					  !HasViceBP(addrCursor, VBP_Break), VBP_Break, false);
		}
	}
	...
	if (sY<0) {
		...
		for (int line = 0; line<(-sY); ++line) {
			--addr;
			int len = 1;
			while (addr && InstructionBytes(addr)>len) {
				--addr;
				++len;
			}
		}
	...
	while (lineNum<lines) {
		...
		int branchTrg = -1;
		int bytes = Disassemble(read, line.end(), line.left(), chars, branchTrg, showBytes, true, showLabels);
		if (editAsmAddr==read&&!editAsmDone) {
			...
			if (ImGui::InputText(editID.c_str(), editAsmStr, sizeof(editAsmStr), ImGuiInputTextFlags_EnterReturnsTrue)) {
				int size = Assemble(editAsmStr, editAsmAddr);
				if (!size) {
					editAsmAddr = -1;
					ForceKeyboardCanvas("DisAsmView");
				} else {
			...
			if (addrCursor>=read && addrCursor<(read+bytes)) {
				if (ImGui::IsKeyPressed(GLFW_KEY_F6)) {
					if (ImGui::IsKeyDown(GLFW_KEY_LEFT_SHIFT)||ImGui::IsKeyDown(GLFW_KEY_RIGHT_SHIFT)) {
						CPUReverseTo(addrCursor);
					} else { CPURunTo(addrCursor); }
				}
	...
	if (!fixedAddress&&(goToPC||focusPC)) {
		...
		for (int line = 0; line<3; ++line) {
			...
			int instrLen = 1;
			while (addr) {
				instrLen = InstructionBytes(addr);
