...
static const char* memDefaultAddr = "$0400";
static const char* memDefaultSpan = "0";
...
void MemView::Draw(int index)
{
	...
	if (evalAddress||(fixedAddress && MemoryChange())) {
		SetAddr(ValueFromExpression(address+(fixedAddress ? 1 : 0)));
	...
	if (showHex||showText) {
		...
		// force font spacing
		uint32_t charWid = (uint32_t)(ImGui::GetWindowWidth()/fontCharWidth);
		...
		if (ImGui::IsMouseClicked(0)) {
			...
			if (mousePos.x>=winPos.x && mousePos.y>=winPos.y &&
				...
				cursor[0] = int((mousePos.x-winPos.x)/fontCharWidth);
		...
		for(int lineNum = 0; lineNum < lines; ++lineNum) {
			...
			if (showHex) {
				...
				for (uint32_t c = 0; c<spanWin; ++c) {
					line.append_num(Get6502Byte(bytes++), 2, 16).append(' ');
				}
			}
			if (showText) {
				uint16_t chars = read;
				for (uint32_t c = 0; c<spanWin; ++c) {
					line.push_utf8(0xee00+Get6502Byte(chars++));
				}
			}
		...
		if (active) {
			...
			if (cursorTime>(0.5f*CursorFlashPeriod)) {
				...
				ImGui::SetCursorPos(ImVec2(fontCharWidth * cursor[0], ImGui::GetTextLineHeightWithSpacing() * cursor[1]));
				const ImVec2 p = ImGui::GetCursorScreenPos();
				ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x+fontCharWidth, p.y+fontCharHeight),
					ImColor(255, 255, 255));
				strown<16> curChr;
				if (showHex && cursor[0]<colT) {
					uint8_t b = Get6502Byte(addrValue+(cursor[0]-col0)/3+cursor[1]*spanWin);
					int nib = (cursor[0]-col0)%3;
					if (nib<2) { curChr.append(strref::num_to_char((b>>(nib ? 0 : 4))&0xf)); }
				} else if (showText) {
					uint8_t b = Get6502Byte(addrValue+(cursor[0]-colT)+cursor[1]*spanWin);
